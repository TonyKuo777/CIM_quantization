...
class BaseBlock(nn.Module):
    ...
    def __init__(self, input_channel, output_channel, bitW, bitA, bitO, t = 6, downsample = False):
        ...
        self.relu6_1 = Act6_Q(bitA=bitA)
        # 3x3   depth wise conv
        #self.conv2 = Conv2d_R2Q(c, c, kernel_size = 3, stride = self.stride, padding = 1, bitW = bitW, groups = c, bias = False)
        self.conv2 = nn.Conv2d(c, c, kernel_size = 3, stride = self.stride, padding = 1)
        self.bn2 = nn.BatchNorm2d(c)
        self.relu6_2 = Act6_Q(bitA=bitA)
        # 1x1   point wise conv
    ...
    def forward(self, inputs):
        # main path
        x = self.relu6_1(self.bn1(self.conv1(inputs)))
...
class My_Model(nn.Module):
    def __init__(self, bitW, bitA, bitO, alpha = 1, sub_channel='v', mode='ReRam', classes=100):
        super(My_Model, self).__init__()
        
        # first conv layer 
        self.conv0 = Conv2d_R2Q(3, int(32*alpha), bitW = bitW, kernel_size = 3, stride = 2, padding = 1, bias = False)
        self.bn0 = nn.BatchNorm2d(int(32*alpha))
        self.relu6_0 = Act6_Q(bitA=bitA)
    ...
    def forward(self, inputs):
        ...
        # first conv layer
        x = self.relu6_0(self.bn0(self.conv0(inputs)))
        ...
        # last conv layer
        x = self.relu6_1(self.bn1(self.conv1(x)))
