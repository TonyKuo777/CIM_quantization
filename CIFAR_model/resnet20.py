...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='B', nbits_w=32, nbits_a=32, alpha_bit=32):
        super(BasicBlock, self).__init__()
        #self.conv1 = Conv2dQ(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False, bit=nbits_w)
        self.conv1 = Conv2dQ_cim(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False, bit=nbits_w)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.act1 = ActQ(bit=nbits_a, alpha_bit=alpha_bit)
        
        #self.conv2 = Conv2dQ(planes, planes, kernel_size=3, stride=1, padding=1, bias=False, bit=nbits_w)
        self.conv2 = Conv2dQ_cim(planes, planes, kernel_size=3, stride=1, padding=1, bias=False, bit=nbits_w)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, nbits_w=32, nbits_a=32, alpha_bit=32, num_classes=10):
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, nbits_w=self.nbits_w, nbits_a=self.nbits_a, alpha_bit=self.alpha_bit))
    ...
    def forward(self, x):
        ...
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)['act']             # class <'torch.Tensor'>
...
def resnet20(nbits_w=32, nbits_a=32, alpha_bit=32, num_classes=10):
    return ResNet(BasicBlock, [3, 3, 3], nbits_w, nbits_a, alpha_bit, num_classes)
...
if __name__ == '__main__':
    ...
    '''
    model = My_Model(nbits_w=args.w, nbits_a=args.a, alpha_bit=args.alpha_bit, num_classes=num_classes)
