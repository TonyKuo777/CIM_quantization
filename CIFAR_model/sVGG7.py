...
class My_Model(nn.Module):
    def __init__(self, bitW, bitA, bitO, sub_channel='v', mode='ReRam', classes=10):
        ...
        self.feature = nn.Sequential(
            ...
            nn.BatchNorm2d(16),
            Act_Q(bitA=bitA),
            Conv2d_R2Q(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, bias=False, bitW=bitW, bitO=bitO, sub_channel=sub_channel, mode=mode),
            nn.BatchNorm2d(16),
            Act_Q(bitA=bitA),
            nn.MaxPool2d(kernel_size=2, stride=2),

            Conv2d_R2Q(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=1, bias=False, bitW=bitW, bitO=bitO, sub_channel=sub_channel, mode=mode),
            nn.BatchNorm2d(32),
            Act_Q(bitA=bitA),
            Conv2d_R2Q(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1, bias=False, bitW=bitW, bitO=bitO, sub_channel=sub_channel, mode=mode),
            nn.BatchNorm2d(32),
            Act_Q(bitA=bitA),
            nn.MaxPool2d(kernel_size=2, stride=2),

            Conv2d_R2Q(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False, bitW=bitW, bitO=bitO, sub_channel=sub_channel, mode=mode),
            nn.BatchNorm2d(64),
            Act_Q(bitA=bitA),
            Conv2d_R2Q(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False, bitW=bitW, bitO=bitO, sub_channel=sub_channel, mode=mode),
            nn.BatchNorm2d(64),
            Act_Q(bitA=bitA),
            nn.MaxPool2d(kernel_size=2, stride=2),
        )
        ...
        self.classifier = nn.Sequential(
            Linear_Q(in_features=4*4*64, out_features=128, bias=False, bitW=bitW), 
            Act_Q(bitA=bitA),
            nn.Linear(in_features=128, out_features=classes, bias=False),
        )
...
if __name__ == '__main__':
    ...
    # Chekpoint save path
    DIR =  './logs/smallvgg7_' + args.dataset + '_w' + str(args.w) + 'a' + str(args.a) + '_sa' + str(args.sa) + '_sub_c' + args.sub_c
