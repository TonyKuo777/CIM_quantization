...
def conv_bn(inp, oup, stride, bitW=32, bitA=32, bitO=32, sub_channel='v'):
    return nn.Sequential(
        ...
        conv_quantize(inp, oup, kernel_size=3, stride=stride, padding=1, bias=False, bitW=bitW, bitO=bitO, sub_channel=sub_channel),
        nn.BatchNorm2d(oup),
...
def conv_1x1_bn(inp, oup, bitW=32, bitA=32, bitO=32, sub_channel='v'):
    return nn.Sequential(
        ...
        conv_quantize(inp, oup, kernel_size=1, stride=1, padding=0, bias=False, bitW=bitW, bitO=bitO, sub_channel=sub_channel),
        nn.BatchNorm2d(oup),
        #nn.ReLU6(inplace=True)
        act_q(bitA=bitA)
    )
    ...
    def __init__(self, inp, oup, stride, expand_ratio, bitW=32, bitA=32, bitO=32, sub_channel='v'):
        ...
        if expand_ratio == 1:
            self.conv = nn.Sequential(
                ...
                conv_quantize(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False, bitW=bitW, bitO=bitO, sub_channel=sub_channel),
                ...
                conv_quantize(hidden_dim, oup, 1, 1, 0, bias=False, bitW=bitW, bitO=bitO, sub_channel=sub_channel),
                nn.BatchNorm2d(oup),
            )
        else:
            self.conv = nn.Sequential(
                ...
                conv_quantize(inp, hidden_dim, 1, 1, 0, bias=False, bitW=bitW, bitO=bitO, sub_channel=sub_channel),
                nn.BatchNorm2d(hidden_dim),
                #nn.ReLU6(inplace=True),
                act_q(bitA),
...
class MobileNetV2(nn.Module):
    def __init__(self, n_class=1000, input_size=224, width_mult=1., bitW=32, bitA=32, bitO=32, sub_channel='v'):
        ...
        self.features = [conv_bn(3, input_channel, 2, 32, 32, 32, 'v')]
...
def mobilenet_v2(bitW=32, bitA=32, bitO=32, sub_channel='v', pretrained=False):
