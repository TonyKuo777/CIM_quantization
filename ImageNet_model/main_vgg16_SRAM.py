...
#from utils.vgg16_SRAM_LLSQ import *
#from models.imagenet.vggQ import *
#from imagenet_models.vgg_SRAM import *
from imagenet_models.vgg_SRAM_alpha import *
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    DIR =  './logs/alpha_vgg16_sram_ste_test_' + 'w' + str(args.w) + 'a' + str(args.a) + '_sa' + str(args.sa) + '_sub_c' + args.sub_c
    ...
    if args.pretrained:
        if args.sub_c == 'v':    
            model = vgg16_bn(bitW=args.w, bitA=args.a, bitO=args.sa, sub_channel='v', pretrained=True)
        else:
            model = vgg16_bn(bitW=args.w, bitA=args.a, bitO=args.sa, sub_channel=int(args.sub_c), pretrained=True)
    else:
        if args.sub_c == 'v':    
            model = vgg16_bn(bitW=args.w, bitA=args.a, bitO=args.sa, sub_channel='v', adc_list=adc_list)
        else:
            model = vgg16_bn(bitW=args.w, bitA=args.a, bitO=args.sa, sub_channel=int(args.sub_c))
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            #state_dict = {k: v for k, v in checkpoint['state_dict'].items() if model.state_dict()[k].numel() == v.numel()}
            '''
